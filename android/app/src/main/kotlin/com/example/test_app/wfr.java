...
public class wfr {
  ...
  private static final String TAG = wfr.class.getSimpleName();
  ...
  private boolean readHeader() {
    ...
    try {
      ...
      Log.d(TAG, "采样率:" + header.mSampleRate);

      mDataInputStream.read(intValue);
      header.mByteRate = byteArrayToInt(intValue);
      Log.d(TAG, "每秒播放字节数:" + header.mByteRate);

      mDataInputStream.read(shortValue);
      header.mBlockAlign = byteArrayToShort(shortValue);
      Log.d(TAG, "采样一次占字节数:" + header.mBlockAlign);

      mDataInputStream.read(shortValue);
      header.mBitsPerSample = byteArrayToShort(shortValue);
      Log.d(TAG, "比特位:" + header.mBitsPerSample);
